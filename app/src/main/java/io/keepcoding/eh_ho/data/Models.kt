...
data class Post(
    ...
    companion object {
        fun parsePostsList(response: JSONObject): List<Post> {
            val postStream = response.getJSONObject("post_stream")
            val objectList = postStream.getJSONArray("posts")

            val posts = mutableListOf<Post>()

            for (i in 0 until objectList.length()) {
                val parsedPost = parsedPost(objectList.getJSONObject(i))
        ...
        fun parsedPost(jsonObject: JSONObject): Post {
            val date = jsonObject.getString("created_at")
                .replace("Z", "+0000")
            ...
            return Post(
                id = jsonObject.getInt("id").toString(),
                titlePost = jsonObject.getString("username").toString(),
                descriptionContent = jsonObject.getString("cooked").toString(),
                date = dateFormatted,
                picture = jsonObject.getString("avatar_template").toString()
     ...
     * @return { unit: "Hora", amount: 1 }
     **/
    fun getTimeOffsetPost(dateToCompare: Date = Date()): TimeOffset {
        val current = dateToCompare.time
        val diff = current - this.date.time
